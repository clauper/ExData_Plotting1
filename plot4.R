...
# this r skript will produce plot 4
...
# (this is not used) s.project1$Date <- strptime(s.project1$Date, format="%d/%m/%Y")
...
plot (s.project1$Time,s.project1$Global_active_power,xlab="",ylab="Global Active Power", type="l")

plot (s.project1$Time,s.project1$Voltage,ylab="Voltage", xlab="datetime", type="l")

plot(s.project1$Time,s.project1$Sub_metering_1,xlab="",ylab="Energy sub metering", type="l")
...
legend("topright",paste("Sub_metering_",c(1:3),sep=""),lty=1,bty="n",col=c("black", "red", "blue"),bg="white")

plot (s.project1$Time,s.project1$Global_reactive_power,ylab="Global_reactive_power", xlab="datetime",type="l")
